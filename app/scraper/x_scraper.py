...
def scrape_x_trends(db=None, embed_inline: bool = True):
    if db is None:
        db = get_db_client()
    ...
    if not _SCRAPE_LOCK.acquire(blocking=False):
        update_scrape_progress(
            state="running",
            message="Scraper already running. Skipping new run.",
        )
        return {"skipped": True}
    ...
    try:
        ...
        for tweet in tweets:
            try:
                text = sanitize_reference_text(tweet.get("text", ""))
                if not text:
                    continue
